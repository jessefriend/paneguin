...
case "$pm" in
  apt)
    common_packages=(xrdp xorgxrdp xorg dbus-x11 desktop-file-utils xdg-utils curl wget iptables)
    case "${DESKTOP_ENV}" in
      kde)
        desktop_packages=(plasma-desktop plasma-workspace konsole dolphin systemsettings)
        [[ "${INSTALL_XFCE_FALLBACK}" == "1" ]] && fallback_packages=(xfce4 xfce4-goodies xfce4-terminal thunar)
        session_command="startplasma-x11"
        desktop_name="KDE"
        ;;
      xfce)
        desktop_packages=(xfce4 xfce4-goodies xfce4-terminal thunar)
    ...
    ;;
  dnf)
    common_packages=(xrdp xorgxrdp dbus-x11 xdg-utils curl wget iptables)
    case "${DESKTOP_ENV}" in
      kde)
        desktop_packages=(@kde-desktop konsole dolphin)
        [[ "${INSTALL_XFCE_FALLBACK}" == "1" ]] && fallback_packages=(xfce4-session xfce4-panel xfdesktop Thunar xfce4-terminal)
        session_command="startplasma-x11"
        desktop_name="KDE"
        ;;
      xfce)
        desktop_packages=(@xfce-desktop-environment xfce4-terminal Thunar)
        session_command="startxfce4"
        desktop_name="XFCE"
        ;;
      mate)
        desktop_packages=(mate-session-manager mate-terminal caja)
        session_command="mate-session"
        desktop_name="MATE"
        ;;
      lxqt)
    ...
    ;;
  zypper)
    common_packages=(xrdp xorgxrdp dbus-1-x11 xdg-utils curl wget iptables)
    case "${DESKTOP_ENV}" in
      kde)
        desktop_packages=(patterns-kde-kde plasma5-session konsole dolphin6)
        [[ "${INSTALL_XFCE_FALLBACK}" == "1" ]] && fallback_packages=(patterns-xfce-xfce xfce4-terminal Thunar)
        session_command="startplasma-x11"
        desktop_name="KDE"
        ;;
      xfce)
        desktop_packages=(patterns-xfce-xfce xfce4-terminal Thunar)
        session_command="startxfce4"
        desktop_name="XFCE"
        ;;
      mate)
        ...
        ;;
      lxqt)
        desktop_packages=(patterns-lxqt-lxqt qterminal pcmanfm-qt)
        session_command="startlxqt"
        desktop_name="LXQt"
        ;;
      *)
...
install_pkgs "${common_packages[@]}" "${desktop_packages[@]}"
if [[ "${#fallback_packages[@]}" -gt 0 ]]; then
  install_pkgs "${fallback_packages[@]}"
fi
