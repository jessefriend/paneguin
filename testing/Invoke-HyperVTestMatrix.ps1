...
function Export-GuestWslDiagnostics {
    param(
        [System.Management.Automation.Runspaces.PSSession]$Session,
        [object]$Case,
...
'@
    ...
    $desktopCommands = switch ($Case.DesktopEnv) {
        "kde"  { @("startplasma-x11", "plasmashell") }
        "xfce" { @("startxfce4", "xfce4-session") }
        "mate" { @("mate-session", "marco", "mate-panel") }
        "lxqt" { @("startlxqt", "openbox", "lxqt-panel") }
    }
...
"@
    ...
    $startwm = Invoke-GuestWslText -Session $Session -Distro $Case.Distro -RunAsRoot -Command 'if [ -f /etc/xrdp/startwm.sh ]; then sed -n "1,200p" /etc/xrdp/startwm.sh; else echo MISSING; fi'
    $xrdpLog = Invoke-GuestWslText -Session $Session -Distro $Case.Distro -RunAsRoot -Command 'if [ -f /var/log/xrdp.log ]; then tail -n 200 /var/log/xrdp.log; else echo MISSING; fi'
    $sesmanLog = Invoke-GuestWslText -Session $Session -Distro $Case.Distro -RunAsRoot -Command 'if [ -f /var/log/xrdp-sesman.log ]; then tail -n 200 /var/log/xrdp-sesman.log; else echo MISSING; fi'
...
"@
    ...
    Set-Content -Path (Join-Path $CaseResultDir "guest-wsl-diagnostics.txt") -Value ($reportLines -join "`r`n") -Encoding UTF8
}
...
try {
    ...
    foreach ($case in $matrix) {
        ...
        try {
            ...
            if ($session) {
                try {
                    Export-GuestWslDiagnostics -Session $session -Case $case -CaseResultDir $caseResultDir
                } catch {
                    Set-Content -Path (Join-Path $caseResultDir "guest-wsl-diagnostics.txt") -Value ("Failed to collect guest WSL diagnostics: " + $_.Exception.Message) -Encoding UTF8
                }
