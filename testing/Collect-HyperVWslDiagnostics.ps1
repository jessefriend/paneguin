...
try {
    ...
    $linuxUserResult = Invoke-GuestWslText -Session $session -Distro $Distro -Command 'id -un 2>/dev/null || true'
    $linuxUser = (($linuxUserResult.Output -split "`r?`n")[0]).Trim()
    if ([string]::IsNullOrWhiteSpace($linuxUser)) {
        $linuxUser = "unknown"
    }

    $homeDirResult = Invoke-GuestWslText -Session $session -Distro $Distro -Command 'printf %s "$HOME"'
    $homeDir = $homeDirResult.Output.Trim()
    if ([string]::IsNullOrWhiteSpace($homeDir)) {
        $homeDir = "/home/$linuxUser"
    }

    $commandCheckCmd = 'for c in xrdp xauth dbus-run-session dbus-launch startplasma-x11 plasmashell startxfce4 xfce4-session mate-session marco mate-panel startlxqt openbox lxqt-panel; do if command -v "$c" >/dev/null 2>&1; then printf "%s -> %s\n" "$c" "$(command -v "$c")"; else printf "%s -> MISSING\n" "$c"; fi; done'
    ...
    $xorgLog = Invoke-GuestWslText -Session $session -Distro $Distro -RunAsRoot -Command ("latest=`$(ls -1t '$homeDirForBash'/.xorgxrdp.*.log 2>/dev/null | head -n 1); if [ -n ""`$latest"" ]; then echo `$latest; tail -n 200 ""`$latest""; else echo MISSING; fi")
